...
### 1-1. フォルダ構成の整備
- [x] `Assets/Scripts/` フォルダを作成
- [x] `Assets/Scripts/Player/` フォルダを作成
- [x] `Assets/Scripts/Enemy/` フォルダを作成
- [x] `Assets/Scripts/Weapon/` フォルダを作成
- [x] `Assets/Scripts/Common/` フォルダを作成（体力・ダメージなど共通処理）
- [x] `Assets/Scripts/UI/` フォルダを作成
- [x] `Assets/Scripts/GameManager/` フォルダを作成
- [x] `Assets/Prefabs/` フォルダを作成

### 1-2. シーンの基本構成
- [x] `MainGame` シーンに床（Plane）を配置
- [x] 障害物（Cube等）をいくつか配置
- [x] ライティングの基本設定（Directional Lightの配置）

### 1-3. NavMesh の設定
- [x] 🔧**User操作**: パッケージマネージャーから `AI Navigation` パッケージをインストール
  - `Window > Package Manager > Unity Registry > AI Navigation > Install`
- [x] 🔧**User操作**: 床オブジェクトに `NavMeshSurface` コンポーネントを追加
- [x] 🔧**User操作**: NavMeshSurface の `Bake` ボタンを押してNavMeshを焼く
...
- [x] `PlayerMovement.cs` を `Assets/Scripts/Player/` に作成
- [x] フィールド定義:
...
- [x] `Start()`: `CharacterController` の参照を `GetComponent` で取得
- [x] `Update()`:
...
- [x] `MouseLook.cs` を `Assets/Scripts/Player/` に作成
- [x] フィールド定義:
...
- [x] `Bullet.cs` を `Assets/Scripts/Weapon/` に作成
  - `float speed = 40f`（弾速）
...
- [x] `Gun.cs` を `Assets/Scripts/Weapon/` に作成
- [x] フィールド定義:
...
- [x] `PlayerShooting.cs` を `Assets/Scripts/Player/` に作成
- [x] フィールド定義:
...
- [ ] 🔧**User操作**: `muzzleFlash` にパーティクルシステムをセット

### 3-4. マズルフラッシュ演出の追加
- [x] `Gun.cs`    - [x] 3-4. Add Muzzle Flash Effect [x]
        - [x] Update `Gun.cs` to trigger particle system
        - [/] User setup: Create particle effect in Unity [/]
...
- [x] `Health.cs` を `Assets/Scripts/Common/` に作成
- [x] フィールド定義:
  - `int maxHealth = 100`（最大体力）
  - `int currentHealth`（現在の体力）
- [x] `Start()`: `currentHealth = maxHealth`
- [x] `TakeDamage(int amount)` メソッド:
...
- [x] `EnemyAI.cs` を `Assets/Scripts/Enemy/` に作成
- [x] フィールド定義:
  - `NavMeshAgent agent`
...
- [x] `GameManager.cs` を `Assets/Scripts/GameManager/` に作成
- [x] シングルトンパターンの実装:
...
### 6-3. クロスヘア（照準）の作成
- [/] 🔧**User操作**: Canvas 内に中心の点（Image）を作成
  - `Image` を作成し、サイズを小さく（10x10など）設定
  - 色を赤や白などの目立つ色に設定
  - `RectTransform` で中央（Center/Middle）に固定
- [ ] 🔧**User操作**: 弾を撃った際の着弾点がクロスヘアの中心と一致するか調整

### 6-4. HUD（ヘッドアップディスプレイ）の実装（オプション）
- [x] `HUDManager.cs` を `Assets/Scripts/UI/` に作成
  - `Text ammoText`（弾数表示用）
